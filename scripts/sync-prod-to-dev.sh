...
#
# Refuses to clobber a juice.duckdb that a local `juice serve` is
...
if [ "$force" -ne 1 ]; then
  if pgrep -fa "juice serve" >/dev/null 2>&1; then
    echo "A local 'juice serve' appears to be running — stop it first" >&2
