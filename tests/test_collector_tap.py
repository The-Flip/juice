...
This is the half of the collector that decides which machine is which. The
cloud recorder used to do it in `refresh_metadata` from a device it had just
polled; here the same work is driven by a frame, which changes one thing that
matters a great deal: the frame can arrive before juice has ever spoken to
FlipFix.
"""
