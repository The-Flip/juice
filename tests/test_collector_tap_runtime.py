...
class TestIngestSkipAtStartup:
    def test_it_moves_cursors_forward_and_never_back(self, store, caplog) -> None:
        from juice.collector_tap import skip_ingest_to

        store.set_ingest_cursor("bumper", "buf-1", "000000000000060000")
        with caplog.at_level(logging.INFO, logger="juice.collector_tap"):
            skip_ingest_to(
                store,
                {("bumper", None): "000000000000090000", ("ghost", None): "000000000000000001"},
            )
            skip_ingest_to(store, {("bumper", None): "000000000000010000"})

        assert store.ingest_cursor("bumper", "buf-1") == "000000000000090000"
        messages = [r.getMessage() for r in caplog.records]
        assert any("moved tap bumper" in m for m in messages)
        assert any("no stored cursor for tap ghost" in m for m in messages)
        assert any("already at" in m for m in messages)

    def test_a_short_cursor_is_refused_not_stored(self, store, caplog) -> None:
        """Cursors compare as fixed-width strings: `90000` stored as newer than
        `000000000000060000` would sort above every real cursor after it and
        ingest would refuse every batch as a duplicate, forever."""
        from juice.collector_tap import skip_ingest_to

        store.set_ingest_cursor("bumper", "buf-1", "000000000000060000")
        with caplog.at_level(logging.ERROR, logger="juice.collector_tap"):
            skip_ingest_to(store, {("bumper", None): "90000"})
        assert store.ingest_cursor("bumper", "buf-1") == "000000000000060000"
        assert any("fixed-width" in r.getMessage() for r in caplog.records)

    def test_an_ambiguous_tap_needs_its_buffer_named(self, store, caplog) -> None:
        """A cursor orders rows only inside one (tap_id, buffer_id) sequence;
        advancing the wrong buffer skips rows it never sent."""
        from juice.collector_tap import skip_ingest_to

        store.set_ingest_cursor("bumper", "buf-old", "000000000000060000")
        store.set_ingest_cursor("bumper", "buf-new", "000000000000000100")
        with caplog.at_level(logging.ERROR, logger="juice.collector_tap"):
            skip_ingest_to(store, {("bumper", None): "000000000000090000"})
        assert store.ingest_cursor("bumper", "buf-old") == "000000000000060000"
        assert store.ingest_cursor("bumper", "buf-new") == "000000000000000100"
        assert any("name one as tap_id:buffer_id" in r.getMessage() for r in caplog.records)

        skip_ingest_to(store, {("bumper", "buf-new"): "000000000000000500"})
        assert store.ingest_cursor("bumper", "buf-new") == "000000000000000500"
        assert store.ingest_cursor("bumper", "buf-old") == "000000000000060000"
